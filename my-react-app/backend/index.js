...
/** Lowercase trimmed email or null if invalid (matches frontend `isValidEmailFormat`). */
...
/** Same rules as `src/passwordRules.ts`. */
...
// Password reset (token in SQLite; dev responses include token — no outbound email in this demo)
...
// Shared catalog + carts (SQLite) — any device using this API sees the same data
...
app.patch('/api/listings/:id/stock', async (req, res) => {
    try {
        ...
        if (!Number.isFinite(delta) || !Number.isInteger(delta)) {
            return res.status(400).json({ message: 'Invalid stock delta' });
        }
...
// --- Stripe Checkout: card entry happens only on Stripe’s hosted Checkout page (PCI-friendly). ---
// FRONTEND_URL must match the Vite dev (or prod) origin used in success_url / cancel_url.
const FRONTEND_URL = process.env.FRONTEND_URL || 'http://localhost:5173';
// Secret key stays on the server only; never expose to the React app.
